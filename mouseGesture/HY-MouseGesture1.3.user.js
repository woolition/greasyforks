...
const MouseGesture = (function() {
    ...
    function showGestureNotDefineTips() {
        tips.innerHTML = symbolTrack + '<br/>' + (cfg.usr.funNotDefine);
            ...
            if (setting[i].type) {
                ...
                if (setting[i].data.type === 'input') {
                    if (setting[i].data.more === 'color') {
                        span = `<input type="text" name="${setting[i].data.name}" value="${GM_getValue(setting[i].data.name,cfg.usr[setting[i].data.name])}" style="background:#${GM_getValue(setting[i].data.name,cfg.usr[setting[i].data.name])};"  data-mark="color">`;
